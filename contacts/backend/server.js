...
const { DB_URI } = process.env; // to grad the same variable from the .env file
...
// Middleware
server.use(express.json()); // To ensure data is tranmitted as JSON
server.use(express.urlencoded({ extended: true })); // To ensure data is encoded and decoded while transmitting
server.use(cors());
...
server.get("/contacts", async (require, response) => {
  try {
